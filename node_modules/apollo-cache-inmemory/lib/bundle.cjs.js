...
var IntrospectionFragmentMatcher = (function () {
    ...
    IntrospectionFragmentMatcher.prototype.match = function (idValue, typeCondition, context) {
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(this.isReady, 10) : tsInvariant.invariant(this.isReady, 'FragmentMatcher.match() was called before FragmentMatcher.init()');
        var obj = context.store.get(idValue.id);
        if (!obj) {
            return idValue.id === 'ROOT_QUERY';
        }
        process.env.NODE_ENV === "production" ? tsInvariant.invariant(obj.__typename, 11) : tsInvariant.invariant(obj.__typename, "Cannot match fragment because __typename property is missing: " + JSON.stringify(obj));
        if (obj.__typename === typeCondition) {
            return true;
        }
...
var StoreReader = (function () {
    function StoreReader(_a) {
        var _b = _a === void 0 ? {} : _a, _c = _b.cacheKeyRoot, cacheKeyRoot = _c === void 0 ? new optimism.KeyTrie(apolloUtilities.canUseWeakMap) : _c, _d = _b.freezeResults, freezeResults = _d === void 0 ? false : _d;
        var _this = this;
    ...
    StoreReader.prototype.diffQueryAgainstStore = function (_a) {
        ...
        if (hasMissingFields && !returnPartialData) {
            execResult.missing.forEach(function (info) {
                if (info.tolerable)
                    return;
                throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(2) : new tsInvariant.InvariantError("Can't find field " + info.fieldName + " on object " + JSON.stringify(info.object, null, 2) + ".");
            });
        }
        if (previousResult) {
        ...
        selectionSet.selections.forEach(function (selection) {
            ...
            if (apolloUtilities.isField(selection)) {
                ...
                if (apolloUtilities.isInlineFragment(selection)) {
                    ...
                    if (!fragment) {
                        throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(3) : new tsInvariant.InvariantError("No fragment named " + selection.name.value);
                    }
                }
    ...
    StoreReader.prototype.executeField = function (object, typename, field, execContext) {
        ...
        if (!field.selectionSet) {
            ...
            if (this.freezeResults && process.env.NODE_ENV !== 'production') {
                apolloUtilities.maybeDeepFreeze(readStoreResult);
            }
...
function assertSelectionSetForIdValue(field, value) {
    if (!field.selectionSet && apolloUtilities.isIdValue(value)) {
        throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(4) : new tsInvariant.InvariantError("Missing selection set for object of type " + value.typename + " returned for query field " + field.name.value);
    }
}
...
function assertIdValue(idValue) {
    process.env.NODE_ENV === "production" ? tsInvariant.invariant(apolloUtilities.isIdValue(idValue), 5) : tsInvariant.invariant(apolloUtilities.isIdValue(idValue), "Encountered a sub-selection on the query, but the store doesn't have an object reference. This should never happen during normal use unless you have custom code that is directly manipulating the store; please file an issue.");
}
        ...
        selectionSet.selections.forEach(function (selection) {
            ...
            if (apolloUtilities.isField(selection)) {
                ...
                if (apolloUtilities.isInlineFragment(selection)) {
                    ...
                    fragment = (fragmentMap || {})[selection.name.value];
                    process.env.NODE_ENV === "production" ? tsInvariant.invariant(fragment, 6) : tsInvariant.invariant(fragment, "No fragment named " + selection.name.value + ".");
                }
                var matches = true;
    ...
    StoreWriter.prototype.writeFieldToStore = function (_a) {
        ...
        if (!field.selectionSet || value === null) {
            ...
            if (dataIdFromObject) {
                ...
                process.env.NODE_ENV === "production" ? tsInvariant.invariant(!semanticId || !isGeneratedId(semanticId), 7) : tsInvariant.invariant(!semanticId || !isGeneratedId(semanticId), 'IDs returned by dataIdFromObject cannot begin with the "$" character.');
                if (semanticId ||
            ...
            if (escapedId !== storeValue && apolloUtilities.isIdValue(escapedId)) {
                ...
                process.env.NODE_ENV === "production" ? tsInvariant.invariant(!generated || escapedId.generated || typenameChanged, 8) : tsInvariant.invariant(!generated || escapedId.generated || typenameChanged, "Store error: the application attempted to write an object with no provided id but the store already contains an id of " + escapedId.id + " for this object. The selectionSet that was trying to be written is:\n" + JSON.stringify(field));
                process.env.NODE_ENV === "production" ? tsInvariant.invariant(!hadTypename || hasTypename, 9) : tsInvariant.invariant(!hadTypename || hasTypename, "Store error: the application attempted to write an object with no provided typename but the store already contains an object with typename of " + escapedId.typename + " for the object of id " + escapedId.id + ". The selectionSet that was trying to be written is:\n" + JSON.stringify(field));
...
var InMemoryCache = (function (_super) {
    ...
    function InMemoryCache(config) {
        ...
        _this.cacheKeyRoot = new optimism.KeyTrie(apolloUtilities.canUseWeakMap);
    ...
    InMemoryCache.prototype.evict = function (query) {
        throw process.env.NODE_ENV === "production" ? new tsInvariant.InvariantError(1) : new tsInvariant.InvariantError("eviction is not implemented on InMemory Cache");
    };
